...
,   order_volume_by_zone AS (
    SELECT
        ...
        ,   COUNT(DISTINCT CASE WHEN t2.order_accepted_datetime IS NULL THEN order_id END)                      AS cnt_delivery_cancel_unavailable_driver -- Assume that order which doesn't has accepted time = Unavailable driver can be found
...
,   delivery_zone_profile AS (
    ...
    LEFT JOIN customer_late_complain_agg t2
)
,   delivery_zone_agg AS (
    SELECT
            delivery_zone
        ,   COALESCE(t1.city, 'all')                                                    AS city
        ...
        ,   (1.00 * SUM(sum_delivery_distance_km)) / SUM(cnt_complete_order * 3600)     AS avg_delivery_spped_kmh
